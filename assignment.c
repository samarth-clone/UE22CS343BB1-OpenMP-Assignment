        ...
        while (1)
        {
                ...
                messageBuffers[threadId].head != messageBuffers[threadId].tail)
            {
                ...
                case FLUSH:
                    ...
                    // check if second receiver set
                    if (threadId == msg.secondReceiver)
                    {
                        // now sharing data, hence update mem an set state to shared
                        if (node.cache[cacheIndex].address != msg.address && node.cache[cacheIndex].state != INVALID)
                        {
                            handleCacheReplacement(threadId, node.cache[cacheIndex]);
                        }
...
// block into cache
                ...
                case WRITEBACK_INV:
                    ...
                    // Makes sense only to flush if hasnt been replaced? (update: idk trying to remove this)
                    // if (node.cache[cacheIndex].address == msg.address && node.cache[cacheIndex].state != INVALID)
                    // {
                    //     msgReply = (message){
                    //         .type = FLUSH_INVACK,
